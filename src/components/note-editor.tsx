...
export function NoteEditor({ note, onUpdate, onDelete, onClose }: NoteEditorProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-white/85">
      <div className="flex items-center justify-between p-6 border-b border-primary/10">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <Button variant="ghost" size="icon" onClick={onDelete} className="text-primary/40 hover:text-red-600 transition-colors">
            <Trash2 className="w-5 h-5" />
          </Button>
          <Button onClick={handleSave} className="rounded-full px-6 bg-primary text-white font-bold" disabled={isSaving}>
            {isSaving ? <Check className="w-4 h-4" /> : "Finish"}
          </Button>
          <Button variant="ghost" size="icon" onClick={onClose} className="rounded-full text-primary/60 hover:text-primary">
            <X className="w-6 h-6" />
          </Button>
        </div>
      </div>
      ...
      <div className="flex-1 p-8 flex flex-col gap-4 overflow-hidden">
        <Textarea
          ...
          className="flex-1 border-none resize-none p-0 focus-visible:ring-0 text-lg leading-relaxed text-primary font-medium bg-transparent"
        />
      </div>

      <div className="p-6 border-t border-primary/10 flex justify-between items-center bg-white/10">
        <div className="text-[10px] text-primary/50 font-bold uppercase tracking-widest">
          {note.updatedAt?.seconds ? new Date(note.updatedAt.seconds * 1000).toLocaleString() : 'Saving...'}
        </div>
        <Button 
          variant="secondary" 
          size="sm" 
